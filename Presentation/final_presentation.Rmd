...
* We will understand image Reconstruction as a High Dimensional Problem.

* We will see how motivation to use Bayesian Methods comes naturally.
...
```{r ,warning=FALSE,echo=FALSE,out.width='75%',fig.align='center',echo=FALSE,fig.cap= "Figure: High and Low Resolution Image"}
...
* We assume IID prior on image gradients, i.e.
  ...
  $$\pi(\boldsymbol{l}) \propto \text{exp}(-\lambda\sum_{i}\sum_{k \ \in \ \{v,h\}} |\delta_k \otimes \boldsymbol{l})_i|^{\alpha})$$
Where,

  * $\lambda > 0$ is a constant determining variance of prior distribution.
...
* It boils down to system of linear equations of form $T_{k,x} x = b$.
...
* In this case $\boldsymbol{k}$ is also unknown $\implies$ more ill possed.

* Finding both $\boldsymbol{k}$ and $\boldsymbol{l}$ together can give rise to trivial solutions.

$$\boldsymbol{l} = \boldsymbol{b} \ \text{and} \ \boldsymbol{k} = 1$$$

* First estimate $\boldsymbol{k}$ and based on that estimate $\boldsymbol{l}$.

* We first express the blurring model in terms of image gradients.
